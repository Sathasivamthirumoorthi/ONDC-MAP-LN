<script lang="ts">
  ...
  if (fromLocation.length > 0 && toLocation.length > 0) {
    ...
    try {
      ...
      const response = await fetch(`https://routing.openstreetmap.de/routed-${travelMode}/route/v1/driving/${from.join(',')};${to.join(',')}?overview=false&alternatives=true&steps=true`);
...
<AppSidebar>
  ...
  <div class="form grid grid-cols-1 gap-4">
    ...
    <div>
      ...
      <button type="button" on:click={() => updateRoute("car")} class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 me-2 mb-2">
        ...
        <button type="button" on:click={() => updateRoute("bike")} class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 me-2 mb-2">
          ...
          <button type="button" on:click={() => updateRoute("foot")} class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 me-2 mb-2">
  ...
  <div>
    ...
    <ul>
      {#each $steps as step}
        <li>
          <strong>{step.type} {step.modifier ? `(${step.modifier})` : ''}</strong> - {step.name} ({step.distance} meters, {step.duration} seconds) via {step.mode}
        </li>
      {/each}
    </ul>
  </div>
